function plotData_nostro(x0,u0,ref,Ts,info,nlobj, ostacoli, rb_mat_int, rb_mat_ext)
    ...
    subplot(6,1,2);
    plot(0:Ts:nlobj.PredictionHorizon*Ts, (info.MVopt(:,2)));
    ylabel('omega (°/s)')
    grid on;
    ylim([-180 180]);
    ...
    subplot(6,1,5);
    plot(0:Ts:nlobj.PredictionHorizon*Ts, (info.Xopt(:,3)));
    ylabel('theta (°)')
    yline(rad2deg(ref(3)),'-.r');
    grid on;
    
    subplot(6,1,6);
    plot(0:Ts:nlobj.PredictionHorizon*Ts, (info.Xopt(:,4)));
    ylabel('psi (°)')
    yline(rad2deg(ref(4)),'-.r');
    grid on;
end
